        ...
        columns: [
          {
            ...
            render(item) {
              return `<h2 class="table-avatar">
                <a class="avatar avatar-sm mr-2"><img class="rounded" src="${item.cover}"></a>
                <a>${item.title} <span>创建人: 小吴</span></a>
                </h2>`;
            },
          },
          {
          ...
          },
          {
            ...
            render(item) {
              let close = '';
              if (item.status !== 3) {
                close = `<a @click="modal('/admin/live/close/${item.id}','是否要关闭该直播间？')" 
                class="btn btn-sm bg-warning text-white">关闭直播间</a>`;
              }
              return `
              <div class="actions btn-group btn-group-sm">
              <a @click="openInfo('/admin/live/look/${item.id}','观看记录')" 
              class="btn btn-sm bg-primary text-white">观看记录</a>
              <a @click="openInfo('/admin/live/gift/${item.id}','礼物记录')" 
              class="btn btn-sm bg-purple text-white">礼物记录</a>
              <a @click="openInfo('/admin/live/comment/${item.id}','弹幕记录')" 
              class="btn btn-sm bg-purple text-white">弹幕记录</a>
              ${close}
              <a @click="del('/admin/live/delete/${item.id}')" 
              class="btn btn-sm bg-danger text-white">删除</a>
              </div>`;
            },
          },
        ],
